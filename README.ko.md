...
이 저장소는 무료 Pixel Agents VS Code 확장의 소스 코드입니다. [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=pablodelucca.pixel-agents) 또는 [Open VSX](https://open-vsx.org/extension/pablodelucca/pixel-agents)에서 설치할 수 있으며, 전체 가구/오피스 에셋이 함께 포함되어 있습니다.
...
사용만 하려면 [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=pablodelucca.pixel-agents)에서 설치하는 것이 가장 쉽습니다. 코드를 직접 수정하거나 기여하려면 아래 단계를 따르세요.
...
> 현재 참고 사항: 저장소 루트의 `npm run check-types`는 transitive type 패키지
> (`@types/d3-array`, `@types/recharts`)에서 오는 기존 DOM-lib 오류 때문에 아직 신뢰할 수 있는 통과 기준이 아닙니다.
> 지금 기준에서는 위의 lint / webview test / webview build 조합을 실전 검증 경로로 보는 것이 좋습니다.
