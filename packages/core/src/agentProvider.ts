...
import { normalizeTranscriptLine } from './transcriptNormalizer.js';
...
export interface SessionProviderAdapter {
  ...
  parseRecord: (agentId: number, line: string, state: ParserAgentState) => NormalizedAgentEvent[];
}
...
export function resolveProviderAdapter(
  ...
  if (id === AGENT_PROVIDER_ID.CUSTOM) {
    ...
    return {
      ...
      parseRecord: normalizeTranscriptLine,
    };
  }
