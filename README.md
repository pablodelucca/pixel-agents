...
Each Claude Code or GitHub Copilot CLI terminal you open spawns a character that walks around, sits at desks, and visually reflects what the agent is doing — typing when writing code, reading when searching files, waiting when it needs your attention.

This is a fork of the [Pixel Agents extension](https://marketplace.visualstudio.com/items?itemName=pablodelucca.pixel-agents) with added support for **GitHub Copilot CLI**.
...
- **Dual AI agent support** — works with both Claude Code CLI and GitHub Copilot CLI
...
- **Diverse characters** — 6 diverse characters with automatic palette assignment
...
- **Either**:
  - [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) installed and configured, **OR**
  - [GitHub Copilot CLI](https://github.com/features/copilot/cli) (`npm install -g @github/copilot`) installed and configured
...
2. **(Optional)** Change agent type in Settings:
   - Click the **Settings** button
   - Toggle between "Claude" and "Copilot"
   - Reload the window when prompted
3. Click **+ Agent** to spawn a new terminal and its character
4. Start coding with Claude or Copilot — watch the character react in real time
...
**Note**: Default agent type is Claude. Switch to Copilot in Settings if you want to use GitHub Copilot CLI.
...
Pixel Agents watches AI agent JSONL transcript files to track what each agent is doing. When an agent uses a tool (like writing a file or running a command), the extension detects it and updates the character's animation accordingly. No modifications to the CLI tools are needed — it's purely observational.
...
- ✅ **Linux**: Should work (uses standard Node.js APIs)
- ⚠️ **Windows**: Should work, Copilot CLI requires WSL
...
- **Heuristic-based status detection** — AI agent transcript formats do not always provide clear signals for when an agent is waiting for user input or when it has finished its turn. The current detection is based on heuristics (idle timers, turn-duration/turn-end events) and may occasionally misfire.
- **GitHub Copilot CLI on Windows** — requires WSL (Windows Subsystem for Linux) as per official Copilot CLI requirements.
