...
export const CLAUDE_PERMISSION_EXEMPT_TOOLS = new Set(['Task', 'AskUserQuestion']);
...
export function processClaudeTranscriptLine(
  ...
  try {
    ...
    if (
      ...
      Array.isArray((record.message as Record<string, unknown>)?.content)
    ) {
      ...
      if (hasToolUse) {
        ...
        if (hasNonExemptTool) {
          startPermissionTimer(
            ...
            agents,
            permissionTimers,
            CLAUDE_PERMISSION_EXEMPT_TOOLS,
            webview,
          );
        }
      ...
      processClaudeProgressRecord(agentId, record, agents, permissionTimers, webview);
  ...
  if (msgType === 'assistant') {
    ...
    for (const block of content) {
      if (
        ...
        const status = formatClaudeToolStatus(toolName, (b.input as Record<string, unknown>) || {});
        console.log(
    ...
    for (const block of content) {
      ...
      if (b.type === 'tool_result' && b.tool_use_id) {
        ...
        if (subTools) {
          subTools.delete(b.tool_use_id as string);
        }
        const subNames = agent.activeSubagentToolNames.get(parentToolId);
        if (subNames) {
          subNames.delete(b.tool_use_id as string);
        }
