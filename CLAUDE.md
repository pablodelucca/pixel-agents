...
**UI styling**: Pixel art aesthetic — all overlays use sharp corners, solid backgrounds, borders, hard offset shadows (`2px 2px 0px #0a0a14`, no blur). CSS variables defined in `index.css` `:root`. Pixel font: FS Pixel Sans (`webview-ui/src/fonts/`), loaded via `@font-face` in `index.css`, applied globally. **Always use Tailwind classes** for styling — never use inline `style={{}}` for static values. Inline styles are only acceptable for truly dynamic values (e.g., a user-chosen color). The project uses `--spacing: 1px` so Tailwind units map directly to pixels (`w-60` = 60px). Reusable scrollbar styling is available via the `pixel-scrollbar` CSS class.
