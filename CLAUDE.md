...
## Orchestrator (Phase 1 — headless delegation)

- `server/src/orchestrator/` — `RoleStore` + `HeadlessSpawner` (standalone, no vscode deps)
- `src/orchestratorManager.ts` — extension-side: creates AgentState `isHeadless: true`, registers with HookEventHandler, spawns `claude -p` subprocess with `--append-system-prompt`, `--permission-mode bypassPermissions`. Attaches `error` + `exit` handlers. `forceDespawn()` cleans up ghost agents when JSONL never appears (30s timeout).
- Command `Pixel Agents: Delegate Task to Role` (id `pixel-agents.delegateTask`): QuickPick over roles from `~/.pixel-agents/roles.json` (fallback: bundled `dist/assets/default-roles.json`), InputBox for task text, spawns headless agent via `OrchestratorManager.delegate()`
- Existing hook infrastructure animates the character in the office — the webview `agentCreated` handler accepts optional `palette` + `hueShift` to honor role-specific skins
- Caller contract: `HeadlessSpawner.spawn()` returns a `ChildProcess` whose `error` and `exit` events fire asynchronously — any consumer MUST attach listeners
- Future phases (not yet implemented): LLM meta (Haiku decomposition), chat/slash commands in webview, dynamic role creation, guardrails (max agents, timeout, approval modal)
