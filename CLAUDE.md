...
**Adding new furniture**: `furniture-catalog.json` is the source of truth when PNG assets exist. The dynamic catalog (`buildDynamicCatalog()`) completely replaces the hardcoded `FURNITURE_CATALOG` fallback. To add a new furniture item: (1) Create a 16-aligned PNG in `webview-ui/public/assets/furniture/{category}/` (2) Add an entry to `furniture-catalog.json` with id, name, label, category, file, width, height, footprintW, footprintH, isDesk, and optional fields (groupId, orientation, state, canPlaceOnSurfaces, backgroundTiles, canPlaceOnWalls). The 8 original hand-drawn sprites (desk, bookshelf, plant, cooler, whiteboard, chair, pc, lamp) are shipped as `DEFAULT_*.png` files generated by `scripts/export-default-furniture.ts`. Their IDs match the `FurnitureType` values for backward compatibility with existing layouts. The hardcoded `FURNITURE_CATALOG` in `furnitureCatalog.ts` is only a fallback for development builds without PNG assets.
